...
@section('content')
    <h2 class="text-2xl font-bold mb-6 text-gray-800 dark:text-white">Edit Platform</h2>
    <form action="{{ route('platforms.update', $platform->id) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="url" name="url" value="{{ $platform->url }}"
                    class="w-full mt-1 p-2 border border-gray-300 rounded-lg dark:bg-gray-900 dark:border-gray-700 focus:ring-2 focus:ring-blue-500 focus:outline-none">
            </div>
            ...
            <div class="col-span-full">
                ...
                @if($platform->logo)
                    <img src="{{ asset('storage/' . $platform->logo) }}" class="h-16 mt-3 rounded shadow">
                @endif
            </div>
        </div>
