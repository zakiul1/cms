...
@section('content')
    <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-lg shadow"
        x-data="{ imageUrl: '{{ $banner->image ? asset('storage/' . $banner->image) : '' }}' }">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-4">Edit Banner</h2>
        ...
        <form method="POST" action="{{ route('banners.update', $banner->id) }}" enctype="multipart/form-data">
            ...
            <div class="mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="button1_text" value="{{ $banner->button1_text }}"
                        class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Button 1 URL</label>
                    <input type="url" name="button1_url" value="{{ $banner->button1_url }}"
                        class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                </div>
            </div>
            ...
            <div class="mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="button2_text" value="{{ $banner->button2_text }}"
                        class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Button 2 URL</label>
                    <input type="url" name="button2_url" value="{{ $banner->button2_url }}"
                        class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                </div>
            </div>
