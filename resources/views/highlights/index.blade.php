...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Highlights</h1>
        <a href="{{ route('highlights.create') }}"
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($highlights as $highlight)
                    <tr>
                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{{ $highlight->title }}</td>
                        <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{{ Str::limit($highlight->content, 80) }}
                        </td>
                        <td class="px-6 py-4">
                            @if($highlight->image)
                                <img src="{{ asset('storage/' . $highlight->image) }}" class="h-12 w-20 object-contain rounded">
                            @else
